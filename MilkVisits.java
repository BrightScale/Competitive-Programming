...
public class MilkVisits {
	...
	private static class Query implements Comparable<Query>{
		int u, v, type, id;
		public Query(int u, int v, int type, int id) {
			this.u = u;
			this.v = v;
			this.type = type;
			this.id = id;
	...
	public static void main(String[] args) throws IOException{
		...
		hld.initTree();
		Query[] qry = new Query[M];
		for(int i = 0; i < M; ++i) qry[i] = new Query(nextInt()-1, nextInt()-1, nextInt(),i);
		Arrays.sort(qry);
		int ans[] = new int[M];
		for(int i = 0; i < M; ++i) {
			while(!pq.isEmpty() && pq.peek().type <= qry[i].type) {
				hld.update(pq.peek().idx,pq.poll().type);
			}
			if(hld.query(qry[i].u, qry[i].v)==qry[i].type) ans[qry[i].id] = 1;
			else ans[qry[i].id] = 0;
		}
	...
	private static class HLD{
		...
		*/
		...
		private static void DFS(int curr, int prev){ //tree traversal without recursion
			++size[curr];
			for(int i : edge[curr]) {
				if(i == prev)continue;
				lca[0][i] = curr; 
