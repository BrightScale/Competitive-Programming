...
public class CowLand {
	...
	public static void main(String[] args) throws IOException{
		st = new StreamTokenizer(new BufferedReader(new FileReader("cowland.in")));
		int N = nextInt(), Q = nextInt();
		int val[] = new int[N];
		for(int i = 0; i < N; ++i) val[i] = nextInt();
		//heavy light decomposition
		HLD hld = new HLD(N);
		for(int i = 1; i < N; ++i) hld.addEdge(nextInt()-1, nextInt()-1);
		hld.initTree(val);
		PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter("cowland.out")));
		for(int i = 0; i < Q; ++i) {
			int t = nextInt();
			if(t==1) hld.update(nextInt()-1, nextInt());
			else pw.println(hld.query(nextInt()-1, nextInt()-1));
		}
		pw.close();
	}
	private static class HLD{
		...
		*/
		...
		public int query(int u, int v) {
			int anc = LCA(u,v);
			return val[anc]^pathQuery(u,anc)^pathQuery(v,anc);
		}
		...
		private static int pathQuery(int child, int par) {
			...
			while(child != par) {
				if(chainHead[child] == child) {
					...
					ret ^= queryST(nodeToSegTree[chainHead[child]],nodeToSegTree[child],0,N-1,0);
					child = lca[0][chainHead[child]];
				}else {
					ret ^= queryST(nodeToSegTree[par]+1,nodeToSegTree[child],0,N-1,0);
					break;
				}
			}
			return ret;
